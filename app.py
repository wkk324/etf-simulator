...
if ticker:
    ...
    if not df_all.empty:
        ...
        if not df.empty:
            ...
            actual_invested = quantity * buy_price  # 최소 매수 시 평가금액 (실제 주식 매수 투입금)
            total_eval = (quantity * sell_price) + (investment_amount - actual_invested)
            eval_profit = total_eval - investment_amount
            eval_profit_pct = (eval_profit / investment_amount) * 100  # 총 평가금액 수익률(%)
            
            st.subheader(f"📌 {selected_etf_label} 시뮬레이션 결과")
            
            # 순서 변경: [매수 주식수] -> [매수 시점 주가] -> [매도 시점 주가(+상승금액/수익률)] -> [최소 매수시 평가금액] -> [최종 평가금액(+평가손익/수익률)]
            ...
            col4.metric("최소 매수시 평가금액", f"{actual_invested:,.0f} 원")
            col5.metric("최종 평가금액", f"{total_eval:,.0f} 원", delta=f"{eval_profit:,.0f} 원 ({eval_profit_pct:.2f}%)")
